...
#print("Courant Number:", a*dt/dx)
#Courant number = 0.3 (which is small)
#exit()
...
def fdm(T):
    ...
    T[0,:] = 5.
    # Neumann at the top
    # cannot do nonlinear here
    T[grid_y-1,:] = T[grid_y-1,:] + d*(T[grid_y-1,:] -4*T[grid_y-1,:] + T[grid_y-1,:] + T[grid_y-2,:] + T[grid_y-2,:]) -f*(2*T[grid_y-1,:] -T[grid_y-1,:] - T[grid_y-2,:])

    T[1:grid_y-1,:] = T[1:grid_y-1,:] + d*(np.roll(T[1:grid_y-1,:],-1,axis=1) -4*T[1:grid_y-1,:] + np.roll(T[1:grid_y-1,:],+1,axis=1) + np.roll(T[2:grid_y,:],-1,axis=0) + np.roll(T[0:grid_y-2,:],+1,axis=0)) -f*(2*T[1:grid_y-1,:] -np.roll(T[1:grid_y-1,:],+1,axis=1) - np.roll(T[0:grid_y-2,:],+1,axis=0))
    return T
...
for n in range(0,nt):
    if n%10==0:
        ...
        ax.text2D(0.05, 0.95, f"Linear, dx = 0.9, a = 30., After %.2f s"%(n*dt), transform=ax.transAxes)
        fig.colorbar(hmmmm)
        plt.savefig('linbetterresolution_%05.5d.png'%(counter))
        plt.clf()
        plt.cla()
