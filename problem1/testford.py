...
d = 0.3 # for neumann stability i need to consider 0 to 0.25
alpha = 5.
len_x = 100
len_y = 100
nt = 1000
...
for n in range(0,nt):
    if n%10==0:
        ...
        ax.text2D(0.05, 0.95, f"d = 0.3 after %.2f s"%(n*dt), transform=ax.transAxes)
        fig.colorbar(hmmmm)
        plt.savefig('0.3d_%05.5d.png'%(counter))
        plt.clf()
        plt.cla()
